export interface CountryCallingCodeDetection {
  callingCode: string;
  countryCode?: string;
}

const CACHE_KEY = 'payool:country-calling-code';

// Used only when IP detection is unavailable. IP detection returns the calling
// code directly, so users outside this fallback list are still supported.
...
function getCachedDetection(): CountryCallingCodeDetection | null {
  try {
    const cached = sessionStorage.getItem(CACHE_KEY);
    return cached ? JSON.parse(cached) as CountryCallingCodeDetection : null;
  } catch {
    return null;
  }
}

function cacheDetection(detection: CountryCallingCodeDetection) {
  try {
    sessionStorage.setItem(CACHE_KEY, JSON.stringify(detection));
  } catch {
...
function getBrowserRegion(): string | undefined {
  ...
  for (const locale of locales) {
    ...
    if (region && COUNTRY_CALLING_CODES[region.toUpperCase()]) {
...
export async function detectCountryCallingCode(signal?: AbortSignal): Promise<CountryCallingCodeDetection | null> {
  const cached = getCachedDetection();
  if (cached?.callingCode) {
    return cached;
  }
  ...
  try {
    ...
    const callingCode = data.country_calling_code?.trim();

    if (callingCode && /^\+\d{1,4}$/.test(callingCode)) {
      const detection = {
        callingCode,
        countryCode: data.country_code?.toUpperCase(),
      };
      cacheDetection(detection);
      return detection;
    }
  } catch {
    // The locale fallback below keeps the form usable offline or if the API is blocked.
  }

  const countryCode = getBrowserRegion();
  if (!countryCode) {
    return null;
  }

  const detection = {
    callingCode: COUNTRY_CALLING_CODES[countryCode],
    countryCode,
  };
  cacheDetection(detection);
  return detection;
